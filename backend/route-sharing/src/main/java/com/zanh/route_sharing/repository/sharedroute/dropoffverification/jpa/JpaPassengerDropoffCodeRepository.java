...
public class JpaPassengerDropoffCodeRepository implements PassengerDropoffCodeRepository {
    ...
    public JpaPassengerDropoffCodeRepository(EntityManager entityManager, DropoffCodeGenerator generator, DropoffCodeProtector protector) {
        this.entityManager = entityManager; this.generator = generator; this.protector = protector;
    }
    ...
    public PassengerDropoffCodeResult getOrCreate(PassengerDropoffCodeCommand command) {
        ...
        try {
            ...
            if (!Objects.equals(booking.getHanhKhach().getId(), command.actorId())) throw codeNotAvailable();
            ...
            if (rows.isEmpty()) {
                ...
                entityManager.persist(ThongTinXacThucTraKhach.activate(trip, booking, dropoff, protectedCode, command.requestedAt()));
                entityManager.flush();
            } else if (rows.size() == 1) {
                ThongTinXacThucTraKhach credential = rows.get(0);
                requireCredentialBinding(credential, trip, booking, dropoff);
                if (!credential.isActive()) throw credentialInvariant();
                code = protector.reveal(credential.protectedCode(), binding);
                requireGeneratedCode(code);
            } else {
                throw credentialInvariant();
            }
            return new PassengerDropoffCodeResult(trip.getId(), booking.getId(), dropoff.getId(), dropoff.getThuTu(), code);
    ...
    private ChuyenDi lockTrip(Long actorId, Long tripId) {
        try {
            return entityManager.createQuery(
                    ...
                    + "where ownRequest.chuyenDi=trip and ownRequest.hanhKhach.id=:actorId)", ChuyenDi.class)
    ...
    private static void requireEligibleTripAndDropoff(ChuyenDi trip, DiemDungHanhTrinh dropoff) {
        if (trip.getTrangThaiVanHanh()!=TrangThaiVanHanhChuyenDi.IN_PROGRESS || trip.getBatDauLuc()==null
                ...
                || dropoff.getBatDauChoLuc()!=null || dropoff.getHanChoLuc()!=null || dropoff.getHoanThanhLuc()!=null) {
            throw codeNotAvailable();
        }
    }

    private static void requireBookingBinding(ChuyenDi trip, DiemDungHanhTrinh dropoff, YeuCauDiChung booking, List<DiemDungHanhTrinh> stops) {
        if (booking==null || booking.getId()==null || booking.getHanhKhach()==null || booking.getHanhKhach().getId()==null
                ...
                || booking.getLyDoXacNhanThatBai()!=null || dropoff.getDenLuc().isBefore(booking.getLenXeLuc())) throw codeNotAvailable();
        List<DiemDungHanhTrinh> pickups=stops.stream().filter(s->s.getLoaiDiemDung()==LoaiDiemDung.PICKUP
                && s.getYeuCauDiChung()!=null && Objects.equals(s.getYeuCauDiChung().getId(),booking.getId())).toList();
        if (pickups.size()!=1 || pickups.get(0).getTrangThaiDiemDung()!=TrangThaiDiemDung.COMPLETED
                || pickups.get(0).getHoanThanhLuc()==null || !pickups.get(0).getHoanThanhLuc().equals(booking.getLenXeLuc())) throw codeNotAvailable();
    }

    private static void requireCredentialBinding(ThongTinXacThucTraKhach c, ChuyenDi t, YeuCauDiChung b, DiemDungHanhTrinh d) {
        if (c.getChuyenDi()==null || c.getYeuCauDiChung()==null || c.getDiemDungHanhTrinh()==null
                || !Objects.equals(c.getChuyenDi().getId(),t.getId()) || !Objects.equals(c.getYeuCauDiChung().getId(),b.getId())
                || !Objects.equals(c.getDiemDungHanhTrinh().getId(),d.getId())) throw credentialInvariant();
    }

    private static void requireGeneratedCode(String code) { if (code==null || !code.matches("[0-9]{6}")) throw credentialInvariant(); }
    private static void requireCommand(PassengerDropoffCodeCommand c) {
        if (c==null || c.actorId()==null || c.actorId()<=0 || c.tripId()==null || c.tripId()<=0 || c.requestedAt()==null)
    ...
    private static BusinessException codeNotAvailable(){return new BusinessException(HttpStatus.CONFLICT,"DROPOFF_CODE_NOT_AVAILABLE","Mã xác nhận trả khách hiện không khả dụng.");}
