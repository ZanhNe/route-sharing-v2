...
public class PostgisTripCompletionRepository implements TripCompletionRepository {
    private static final Set<TrangThaiYeuCau> RESOLVED_BOOKING_STATES = EnumSet.of(TrangThaiYeuCau.COMPLETED,
            TrangThaiYeuCau.NO_SHOW, TrangThaiYeuCau.ABORTED);
    ...
    public TripCompletionCommitResult commit(TripCompletionCommitCommand command) {
        ...
        try {
            ...
            // Critical chronology rule: obtain the business time only after the shared Trip
            // lock is held.
    ...
    private static TripStructure requireTripStructure(ChuyenDi trip, List<DiemDungHanhTrinh> stops) {
        ...
        if (stops == null || stops.size() != expected)
            throw invariantViolation();
        ...
        for (DiemDungHanhTrinh stop : stops) {
            if (stop.getId() == null || stop.getThuTu() == null || stop.getThuTu() <= previousOrder
                    ...
                    || !Wgs84Coordinates.isValidLongitudeLatitude(stop.getToaDoKeHoach().getX(),
                            stop.getToaDoKeHoach().getY())
            ...
            if (stop.getLoaiDiemDung() == LoaiDiemDung.DRIVER_START)
                ...
                if (stop.getYeuCauDiChung() == null || stop.getYeuCauDiChung().getId() == null)
                    throw invariantViolation();
                Map<Long, DiemDungHanhTrinh> target = stop.getLoaiDiemDung() == LoaiDiemDung.PICKUP ? pickups
                        : dropoffs;
                if (target.put(stop.getYeuCauDiChung().getId(), stop) != null)
                    throw invariantViolation();
            }
        }
        if (starts.size() != 1 || ends.size() != 1 || pickups.size() != trip.getSoKhachKeHoach()
                || dropoffs.size() != trip.getSoKhachKeHoach())
            throw invariantViolation();
        ...
        if (stops.get(0) != start || stops.get(stops.size() - 1) != end
                ...
                || !Wgs84Coordinates.isValidLongitudeLatitude(start.getToaDoThucTe().getX(),
                        start.getToaDoThucTe().getY())
                || start.getHoanThanhLuc() == null || !start.getHoanThanhLuc().equals(trip.getBatDauLuc())
                || start.getDenGanLuc() != null || start.getDenLuc() != null || start.getBatDauChoLuc() != null
                || start.getHanChoLuc() != null
                || end.getTrangThaiDiemDung() != TrangThaiDiemDung.PENDING
                || end.getToaDoThucTe() != null || end.getHoanThanhLuc() != null
                || end.getDenGanLuc() != null || end.getDenLuc() != null || end.getBatDauChoLuc() != null
                || end.getHanChoLuc() != null) {
            throw invariantViolation();
        }
    ...
    private static void requirePassengerLifecycleResolution(ChuyenDi trip, List<YeuCauDiChung> bookings) {
        if (bookings == null || bookings.size() != trip.getSoKhachKeHoach())
            throw invariantViolation();
        long onBoardCount = bookings.stream().filter(b -> b.getTrangThaiYeuCau() == TrangThaiYeuCau.ON_BOARD).count();
        if (onBoardCount != trip.getSoKhachThucTe())
            throw invariantViolation();
        if (onBoardCount > 0)
            throw hasOnboardPassengers();
        ...
        if (booking.getTrangThaiYeuCau() == TrangThaiYeuCau.ABORTED) {
            ...
            if (booking.getLenXeLuc() == null) {
                ...
                if (booking.getLenXeLuc().isBefore(booking.getChapNhanLuc())
                        ...
                        || pickup.getDenLuc() == null || pickup.getBatDauChoLuc() == null
    ...
    private void requireNoActiveSafetyHold(Long tripId) {
        Long active = entityManager.createQuery(
                "select count(intervention) from CanThiepAnToanChuyenDi intervention "
                        ...
                        + "and intervention.trangThaiCanThiep=:state",
                Long.class)
                .setParameter("tripId", tripId)
    ...
    private static void requireCommand(TripCompletionCommitCommand command) {
        if (command == null || command.actorId() == null || command.actorId() <= 0
                ...
                || !Wgs84Coordinates.isValidLongitudeLatitude(command.currentLocation().getX(),
                        command.currentLocation().getY())) {
    ...
    private static BusinessException alreadyCompleted() {
        return new BusinessException(HttpStatus.CONFLICT, "TRIP_ALREADY_COMPLETED",
                "Chuyến đi đã được kết thúc bình thường trước đó.");
    }

    private static BusinessException notInProgress() {
        return new BusinessException(HttpStatus.CONFLICT, "TRIP_NOT_IN_PROGRESS",
                "Chuyến đi phải ở trạng thái IN_PROGRESS để kết thúc bình thường.");
    }

    private static BusinessException hasOnboardPassengers() {
        return new BusinessException(HttpStatus.CONFLICT, "TRIP_HAS_ONBOARD_PASSENGERS",
                "Chuyến đi vẫn còn hành khách đang trên xe.");
    }

    private static BusinessException unresolvedPassengerObligations() {
        return new BusinessException(HttpStatus.CONFLICT, "TRIP_PASSENGER_OBLIGATIONS_UNRESOLVED",
                "Chuyến đi vẫn còn nghĩa vụ hành khách chưa được giải quyết.");
    }

    private static BusinessException nextStopNotDriverEnd() {
        return new BusinessException(HttpStatus.CONFLICT, "NEXT_TRIP_STOP_NOT_DRIVER_END",
                "Điểm dừng hiện tại chưa phải điểm kết thúc của tài xế.");
    }

    private static BusinessException outsideEndRadius() {
        return new BusinessException(HttpStatus.CONFLICT, "DRIVER_OUTSIDE_END_RADIUS",
                "Tài xế chưa ở trong phạm vi điểm kết thúc chuyến.");
    }

    private static BusinessException invariantViolation() {
        return new BusinessException(HttpStatus.INTERNAL_SERVER_ERROR, "TRIP_COMPLETION_INVARIANT_VIOLATION",
                "Dữ liệu chuyến đi không nhất quán để kết thúc bình thường.");
    }
    ...
    private static BusinessException dataIntegrityViolation() {
        return new BusinessException(HttpStatus.CONFLICT, "DATA_INTEGRITY_VIOLATION",
                "Không thể kết thúc chuyến do xung đột dữ liệu.");
    }
}
