...
public class DriverSharedRouteResponseMapper {
    ...
    public DriverSharedRouteDetailResponse toDetail(DriverSharedRouteDetailRow row) {
        return new DriverSharedRouteDetailResponse(
                ...
                endpoint(row.destinationLatitude(), row.destinationLongitude(), row.destinationAddress()),
                ...
                vehicle(row.vehicleId(), row.licensePlate(), row.actualColor(), row.brandName(), row.modelName()),
                bookings(row.totalRequests(), row.pendingRequests(), row.acceptedBookings(),
                        row.rejectedRequests(), row.cancelledByPassenger(), row.cancelledByDriver()),
                row.assignedToTrip(),
