...
public final class PostgreSqlSchemaContributor implements AdditionalMappingContributor {
  ...
  static String[] createAfterTables() {
    return new String[] {
        ...
        "CREATE UNIQUE INDEX IF NOT EXISTS uk_ho_so_thanh_vien_hien_hanh "
            + "ON ho_so_thanh_vien (nguoi_dung_id, nha_truong_id) "
            + "WHERE trang_thai_ho_so IN "
            + "('DRAFT','SUBMITTED','UNDER_REVIEW','NEED_SUPPLEMENT','APPROVED','SUSPENDED')",
        "CREATE UNIQUE INDEX IF NOT EXISTS uk_bang_chung_thanh_vien_current_slot "
            + "ON bang_chung_thanh_vien (ho_so_thanh_vien_id, slot) "
            + "WHERE is_current = TRUE",
        "CREATE INDEX IF NOT EXISTS idx_ho_so_sinh_vien_ma_so_normalized "
            + "ON ho_so_sinh_vien (upper(ma_so_sinh_vien)) "
            + "WHERE ma_so_sinh_vien IS NOT NULL",
  ...
  static String blockingRideRequestStatesSql() {
    return Arrays.stream(TrangThaiYeuCau.values())
        .filter(TrangThaiYeuCau::blocksNewRequest)
  ...
  static String[] dropBeforeTables() {
    return new String[] {
        ...
        "DROP INDEX IF EXISTS idx_ho_so_sinh_vien_ma_so_normalized",
        "DROP INDEX IF EXISTS uk_bang_chung_thanh_vien_current_slot",
