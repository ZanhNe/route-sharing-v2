...
/**
 * Đăng ký các đối tượng PostgreSQL/PostGIS mà Jakarta Persistence không thể biểu diễn
 * đầy đủ bằng annotation của Entity, nhưng vẫn phải được tạo/xóa cùng vòng đời schema
 * do Hibernate quản lý.
 *
 * <p>Không có bước chạy SQL thủ công. Khi {@code ddl-auto=create}:</p>
 * <ol>
 *     <li>PostGIS extension được bảo đảm tồn tại trước khi Hibernate tạo bảng geometry.</li>
 *     <li>Sau khi các bảng được tạo, Hibernate tự tạo GiST/expression index và các
 *         function/trigger bảo vệ security version.</li>
 * </ol>
 */
public final class PostgreSqlSchemaContributor implements AdditionalMappingContributor {
    ...
    static String[] createAfterTables() {
        return new String[]{
                ...
                //
                // Không khai báo các index này bằng jakarta.persistence.Index#options.
                // Hibernate 7.4 ghép unique partial index vào CREATE TABLE dưới dạng
                // "constraint ... unique (...) WHERE ...", không phải cú pháp PostgreSQL hợp lệ.
                // Chúng vẫn thuộc schema code-first và được Hibernate tạo sau khi mọi bảng tồn tại.
                ...
                // UC-E1-02 — expression indexes matching the ::geography predicates
                ...
                // V2 — case-insensitive uniqueness. Entity callbacks normalize these values,
                // while these indexes also protect writes performed outside Hibernate.
    ...
    static String[] dropBeforeTables() {
        return new String[]{
                /*
                 * Dùng CASCADE cho các function: PostgreSQL sẽ tự gỡ trigger phụ thuộc.
                 * Cách này an toàn cả khi một lần schema-create trước đó thất bại giữa chừng
                 * và một số bảng không tồn tại.
                 */
